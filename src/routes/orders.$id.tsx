...
import {
  ArrowLeft, Upload, Trash2, FileText, Calendar, User, Phone, Mail, MessageCircle, Globe, MapPin,
} from "lucide-react";
import {
  ORDER_STATUSES, STATUS_LABELS, PRIORITIES, formatVND, logActivity, uploadOrderFile, notify,
} from "@/lib/logger";

export const Route = createFileRoute("/orders/$id")({
  component: () => <AppLayout><OrderDetail /></AppLayout>,
});
...
function OrderDetail() {
  ...
  const historyQ = useQuery({
    ...
    queryFn: async () => (await (supabase as any).from("order_status_history").select("*").eq("order_id", id).order("created_at", { ascending: false })).data ?? [],
  });
  const commentsQ = useQuery({
    queryKey: ["order-comments", id],
    queryFn: async () => (await (supabase as any).from("order_comments").select("*").eq("order_id", id).order("created_at", { ascending: false })).data ?? [],
  });
  const filesQ = useQuery({
    queryKey: ["order-files", id],
    queryFn: async () => (await (supabase as any).from("order_attachments").select("*").eq("order_id", id).order("created_at", { ascending: false })).data ?? [],
  });
  const logsQ = useQuery({
    queryKey: ["order-logs", id],
    queryFn: async () => (await supabase.from("activity_logs").select("*").eq("entity_id", id).order("created_at", { ascending: false })).data ?? [],
  });
  ...
  const remove = async () => {
    ...
    logActivity("order_deleted", `Order for ${o?.client_name} deleted`, "order", id);
    nav({ to: "/orders" });
  };
  ...
  const timeline = [
    ...(historyQ.data ?? []).map((h: any) => ({
      kind: "status",
      ts: h.created_at,
      label: h.from_status ? `Status: ${STATUS_LABELS[h.from_status] ?? h.from_status} → ${STATUS_LABELS[h.to_status] ?? h.to_status}` : `Created as ${STATUS_LABELS[h.to_status] ?? h.to_status}`,
      note: h.note,
    })),
    ...(logsQ.data ?? []).map((l: any) => ({ kind: "log", ts: l.created_at, label: l.message, note: null })),
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          {o.client_avatar_url ? (
            <img src={o.client_avatar_url} alt="" className="h-12 w-12 rounded-lg object-cover border border-border" />
          ) : (
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-semibold tracking-tight">{o.client_name}</h1>
        ...
        <div className="flex items-center gap-2">
          <Select value={o.status} onValueChange={async (v) => {
            ...
            logActivity("status_changed", `Order ${o.client_name} → ${STATUS_LABELS[v] ?? v}`, "order", id);
            await notify("order_updated", `Order status updated`, `${o.client_name}: ${STATUS_LABELS[v] ?? v}`, "order", id);
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="space-y-5 mt-5">
          <div className="grid lg:grid-cols-3 gap-5">
            ...
            <div className="glass rounded-xl p-5 space-y-3">
              ...
              <Row label="Remaining" value={<span className={remaining > 0 ? "text-amber-500" : ""}>{formatVND(remaining)}</span>} />
          ...
          <div className="grid lg:grid-cols-3 gap-5">
            <div className="glass rounded-xl p-5 space-y-3 lg:col-span-2">
              ...
              <div className="flex items-center gap-3">
                <input
                  type="range" min={0} max={100} value={o.progress ?? 0}
              ...
              <div className="h-2 bg-muted rounded-full overflow-hidden">
                <div className="h-full bg-primary transition-all" style={{ width: `${o.progress ?? 0}%` }} />
              </div>
              <div className="grid sm:grid-cols-2 gap-y-2 pt-2 text-sm">
                <Info icon={Calendar} label="Deadline" value={o.deadline ? new Date(o.deadline).toLocaleDateString() : "—"} />
                <Info icon={Calendar} label="Est. delivery" value={o.estimated_delivery ? new Date(o.estimated_delivery).toLocaleDateString() : "—"} />
                <Info icon={FileText} label="Package" value={o.package} />
                <Info icon={FileText} label="Service" value={o.service_category ?? o.service} />
              </div>
            </div>
            ...
            <div className="glass rounded-xl p-5 space-y-3">
              ...
              <Textarea
                ...
                onBlur={(e) => e.target.value !== (o.internal_notes ?? "") && updateField({ internal_notes: e.target.value })}
              />
              <Label className="text-xs">Client-facing notes</Label>
              <Textarea
                rows={3}
                defaultValue={o.client_notes ?? o.notes ?? ""}
                onBlur={(e) => e.target.value !== (o.client_notes ?? o.notes ?? "") && updateField({ client_notes: e.target.value })}
              />
            </div>
          </div>
        </TabsContent>
              ...
              <ol className="space-y-4">
                {timeline.map((t, i) => (
                  <li key={i} className="relative pl-6 border-l border-border last:border-l-transparent">
                    ...
                    <div className="text-[11px] text-muted-foreground">{new Date(t.ts).toLocaleString()}</div>
                  </li>
                ))}
              </ol>
            )}
        ...
        <TabsContent value="comments" className="mt-5">
          <CommentsPanel orderId={id} comments={commentsQ.data ?? []} onChanged={() => qc.invalidateQueries({ queryKey: ["order-comments", id] })} />
        </TabsContent>

        <TabsContent value="files" className="mt-5">
          <FilesPanel orderId={id} files={filesQ.data ?? []} onChanged={() => qc.invalidateQueries({ queryKey: ["order-files", id] })} />
        </TabsContent>
      </Tabs>
    </div>
  );
...
function CommentsPanel({ orderId, comments, onChanged }: any) {
  const [body, setBody] = useState("");
  const [internal, setInternal] = useState(true);
  const send = async () => {
    if (!body.trim()) return;
    const { error } = await (supabase as any).from("order_comments").insert({
      order_id: orderId, body, is_internal: internal,
    });
    if (error) return toast.error(error.message);
    setBody("");
    onChanged();
    ...
    <div className="space-y-4">
      <div className="glass rounded-xl p-4 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button onClick={send} size="sm">Post comment</Button>
        </div>
      </div>
      {comments.length === 0 ? (
        ...
        <div className="space-y-3">
          {comments.map((c: any) => (
            <div key={c.id} className={`rounded-xl p-4 border ${c.is_internal ? "bg-amber-500/5 border-amber-500/20" : "bg-card border-border"}`}>
              <div className="flex items-center justify-between text-xs text-muted-foreground mb-2">
                <span>{c.author_name ?? "Team"} {c.is_internal && <span className="text-amber-500">· internal</span>}</span>
...
function FilesPanel({ orderId, files, onChanged }: any) {
  const [uploading, setUploading] = useState(false);
  const onUpload = async (file: File) => {
    setUploading(true);
    try {
      const { url } = await uploadOrderFile(file);
      const { error } = await (supabase as any).from("order_attachments").insert({
        order_id: orderId, name: file.name, url, size: file.size, mime: file.type,
      });
      if (error) throw error;
  ...
  const del = async (f: any) => {
    ...
    await (supabase as any).from("order_attachments").delete().eq("id", f.id);
    onChanged();
  };
  return (
    <div className="space-y-4">
          ...
          {files.map((f: any) => (
            <div key={f.id} className="glass rounded-xl p-4 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <a href={f.url} target="_blank" rel="noreferrer" className="text-sm font-medium truncate hover:underline block">{f.name}</a>
                <div className="text-xs text-muted-foreground">{f.size ? `${Math.round(f.size / 1024)} KB` : ""} · {new Date(f.created_at).toLocaleDateString()}</div>
              </div>
