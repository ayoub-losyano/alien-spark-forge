...
function OrdersPage() {
  ...
  const filtered = useMemo(() => {
    return (data ?? []).filter((o) => {
      ...
      if (
        search &&
        !`${o.client_name} ${o.company_name ?? ""} ${o.business_name ?? ""} ${o.email ?? ""} ${o.phone ?? ""}`
          .toLowerCase()
  ...
  const del = async (id: string, name: string) => {
    ...
    try {
      await orderService.delete(id, name);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage all client orders and project progress.
          </p>
        </div>
        <Link to="/orders/new">
                ...
                >
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {o.company_name || o.business_name || o.email}
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-1">
                      ...
                      <Button
                        size="icon"
                        variant="ghost"
                        onClick={() => del(o.id, o.client_name)}
                      >
...
function EditOrderDialog({
  ...
  const save = async () => {
    ...
    try {
      ...
      logActivity(
        "order_updated",
        `Order for ${f.client_name} updated`,
        "order",
        order.id
      );
    ...
    <Dialog open={!!order} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl glass">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          ...
          <div>
            <Label className="text-xs">Total (VND)</Label>
            <Input
              type="number"
              value={f?.total ?? 0}
          ...
          <div>
            <Label className="text-xs">Deposit (VND)</Label>
            <Input
              type="number"
              value={f?.deposit ?? 0}
          ...
          <div>
            ...
            <Input
              ...
              value={
                f?.deadline
                  ? new Date(f.deadline).toISOString().slice(0, 10)
                  : ""
              }
              onChange={(e) =>
                set(
