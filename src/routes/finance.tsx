...
import { EXPENSE_CATEGORIES, formatVND, logActivity } from "@/lib/logger";
...
function Finance() {
  ...
  const expensesQ = useQuery({
    queryKey: ["expenses"],
    queryFn: async () => (await supabase.from("expenses").select("*").order("paid_on", { ascending: false })).data ?? [],
  });
  ...
  const monthlySeries = (() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      out.push({ label: d.toLocaleDateString("en-US", { month: "short" }), revenue: r, expense: e });
    }
    return out;
  })();
  const maxMonthly = Math.max(1, ...monthlySeries.map((m) => Math.max(m.revenue, m.expense)));
    ...
    <div className="space-y-8">
      ...
      <div className="glass rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
            <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-sm bg-primary" /> Revenue</span>
          ...
          {monthlySeries.map((m) => (
            <div key={m.label} className="flex flex-col items-center justify-end gap-1">
              <div className="flex items-end gap-1 h-full w-full justify-center">
                <div className="w-3 bg-primary/70 rounded-t" style={{ height: `${(m.revenue / maxMonthly) * 100}%` }} />
                <div className="w-3 bg-destructive/60 rounded-t" style={{ height: `${(m.expense / maxMonthly) * 100}%` }} />
              </div>
      ...
      <div className="glass rounded-xl overflow-hidden">
        ...
        <TabsContent value="expenses">
          <ExpensesPanel expenses={expenses} loading={expensesQ.isLoading} onChanged={() => qc.invalidateQueries({ queryKey: ["expenses"] })} />
        </TabsContent>
      </Tabs>
    </div>
  );
...
function ExpensesPanel({ expenses, loading, onChanged }: { expenses: any[]; loading: boolean; onChanged: () => void }) {
  const [open, setOpen] = useState(false);
  const empty = { description: "", category: "Software", amount: 0, paid_on: new Date().toISOString().slice(0, 10), notes: "" };
  ...
  const save = async () => {
    ...
    logActivity("expense_added", `Expense: ${f.description}`, "expense");
    onChanged();
    setOpen(false);
    reset();
  };
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!loading && expenses.length === 0 && <tr><td colSpan={5} className="text-center py-8 text-muted-foreground">No expenses yet.</td></tr>}
              {expenses.map((e) => (
                <tr key={e.id} className="border-t border-border hover:bg-muted/40 transition-colors">
                  <td className="px-4 py-3 text-muted-foreground">{new Date(e.paid_on).toLocaleDateString()}</td>
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><Label>Amount (VND)</Label><Input type="number" value={f.amount} onChange={(e) => setF({ ...f, amount: e.target.value })} /></div>
            </div>
            <div><Label>Paid on</Label><Input type="date" value={f.paid_on} onChange={(e) => setF({ ...f, paid_on: e.target.value })} /></div>
