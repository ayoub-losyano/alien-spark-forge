...
import { orderService, teamService } from "@/lib/services";
...
function NewOrder() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payment_status = orderService.calculatePaymentStatus(
        Number(f.deposit),
        Number(f.total)
      );
      ...
      await orderService.create({
        ...
        delivery_days: Number(f.delivery_days) || null,
    ...
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Create a new client order or lead.
        </p>
      </div>
      <form onSubmit={submit} className="glass rounded-2xl p-6 space-y-6">
        <section className="grid md:grid-cols-2 gap-4">
          ...
          <Field id="service" label="Service / scope">
            <Input
              ...
              placeholder="e.g. Landing page + chatbot"
            />
          </Field>
        </section>
        ...
        <section className="grid md:grid-cols-3 gap-4">
          ...
          <Field id="tiktok" label="TikTok URL">
            <Input
              id="tiktok"
              value={f.tiktok}
        ...
        <section className="grid md:grid-cols-3 gap-4">
          ...
          <Field id="total" label="Total price (VND)">
            <Input
              id="total"
              type="number"
              value={f.total}
          ...
          <Field id="deposit" label="Deposit paid (VND)">
            <Input
              id="deposit"
              type="number"
              value={f.deposit}
          ...
          <Field id="payment_method" label="Payment method">
            ...
            >
              ...
              <SelectContent>
                {["Bank transfer", "Momo", "ZaloPay", "Cash", "Other"].map(
                  (p) => (
                    <SelectItem key={p} value={p}>
                      {p}
                    </SelectItem>
          ...
          <Field id="delivery_days" label="Delivery (days)">
            <Input
              id="delivery_days"
              type="number"
              value={f.delivery_days}
        ...
        <div className="flex gap-3 justify-end pt-2">
          <Button type="button" variant="ghost" onClick={() => nav({ to: "/orders" })}>
            Cancel
          </Button>
          <Button
            type="submit"
